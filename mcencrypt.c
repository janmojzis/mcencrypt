...
int main(int argc, char **argv) {
    ...
    chacha20_blocks(&g.chacha20ctx, g.ak, g.ak, sizeof g.ak); /* XXX warning - must be power of chacha20_BLOCKBYTES */
    ...
    for (;;) {
        ...
        inputlen += r;
        if (inputlen > MAX) { errno = EPROTO; die_perm(NAME, "read message failed"); } /* chacha20 internal counter limits length to 2^70 bytes */
